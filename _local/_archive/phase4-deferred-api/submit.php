 ...
 *
 * The only unauthenticated thing that writes to disk, so it is the highest
 * risk surface in the project. Every guard here exists for a reason:
 *
 *  - files land OUTSIDE public_html, so an unreviewed upload is never on the
 *    web, and a disguised script is not reachable to be run
 *  - the stored filename is generated, never the uploader's
 *  - a file is a PDF because it STARTS like one, not because of its extension
 *  - the queue is capped, which is what actually bounds disk and inode abuse
 *  - the cooldown is per-browser, NOT per-IP: campus wifi puts thousands of
 *    students behind one address, so per-IP limits either block everyone or
 *    stop nobody
 */
...
$f = $_FILES['file'];
if ($f['size'] <= 0 || $f['size'] > $c['max_upload_bytes']) {
  fail(413, 'That file is too large.');
}
if (!is_uploaded_file($f['tmp_name']) || !looks_like_pdf($f['tmp_name'])) {
  fail(415, 'Only PDF files can be accepted.');
}
...
/* What the student typed is kept as a SUGGESTION. The admin corrects it in
   the console before anything is published, so none of it is trusted here
   beyond being stored as text. */
...
$record = [
  ...
  'course'      => $field('course'),
  'department'  => $field('department', 20),
  'type'        => in_array($_POST['type'] ?? '', ['papers','notes','assignment','reference'], true)
                     ? $_POST['type'] : 'papers',
  'semester'    => $field('semester', 50),
  'year'        => (int) ($_POST['year'] ?? 0) ?: (int) preg_replace('/\D/', '', (string)($_POST['semester'] ?? '')) ?: null,
...
/* Duplicate check spans BOTH the live index and the pending queue — two
   students can submit the same paper before either is reviewed. */
$record['duplicateOf'] = null;
foreach (read_json($c['public_dir'] . '/resources.json', []) as $r) {
